...
新增必须实施的策略：读 docs/REFRESH_POLICY.md 与 config/refresh-policy.json。每个 UTC 日采集全部已登记来源的五分之一，五天（120 小时）覆盖一轮。真实关闭／截止自动下架，标题和主要按钮直达官方申请页。原来只做“站内岗位详情后再点申请”的流程已不满足要求。运行 `services/ingestion/src/worker.py --once`。
